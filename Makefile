...
llvm-init: init
	...
	cd $(PWD)/circt/llvm/build ; cmake -G Ninja ../llvm -DLLVM_ENABLE_PROJECTS=mlir -DCMAKE_BUILD_TYPE=RelWithDebInfo -DLLVM_ENABLE_LLD=ON -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DLLVM_ENABLE_EH=ON -DLLVM_ENABLE_RTTI=ON -DLLVM_INSTALL_UTILS=ON -DLLVM_PARALLEL_LINK_JOBS=2 -DCMAKE_INSTALL_PREFIX="$(PWD)/prefix/" -DLLVM_BUILD_LLVM_DYLIB=ON
...
circt-init: init llvm
	...
	cd $(PWD)/circt/build; cmake -G Ninja .. -DMLIR_DIR="$(PWD)/circt/llvm/build/lib/cmake/mlir" -DLLVM_DIR="$(PWD)/circt/llvm/build/lib/cmake/llvm" -DCMAKE_BUILD_TYPE=RelWithDebInfo -DLLVM_ENABLE_LLD=ON -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_INSTALL_PREFIX="$(PWD)/prefix/"
